...
export default class TraineeProfilesController {
    ...
    static async getAvailableTraineeProfiles(req, res) {
        try {
            ...
            if (id) {
                const results = await client.raw(`SELECT T.id, avatar_url, first_name, last_name, phone, user_id, firebase_uid, email, type, is_activate FROM trainee_profile as T inner join "user" as A on (A.id = T.user_id) WHERE A.is_activate = TRUE and T.user_id = ${id}`);
